...
int test_tm_service_five()
{
	...
	for (int i = 0; i < outTm1.data.u.tmData.data.u.st_5_8.nCount; i++)
	{
		...
		printf("Data: %s   ->   %s\n", outTm1.data.u.tmData.data.u.st_5_8.arr[i].data.arr, outTm2.data.u.tmData.data.u.st_5_8.arr[i].data.arr);
	}
...
int test_tc_service_eighteen()
{
	...
	asn1SccPus30ByteArray rep = { 4, "this" };
	asn1SccPus30ByteArray file = { 5, "thaat" };
...
int test_tm_service_twentythree()
{
	...
	const asn1SccPus30ByteArray src = { 6, "jeness" };
	const asn1SccPus30ByteArray pat[8] = { {4, "asdf"}, {3, "qss"}, {2, "ws"}, {2, "es"}, {2, "rs"}, {2, "ts"}, {5, "zzzzs"}, {3, "abc"} };
...
int test_tc_service_twentythree()
{
	...
	pusApid_t apid = 0x88;
	pusSequenceCount_t seq = 0x88;
	asn1SccPus30ByteArray rep = { 5, "thiis" };
	asn1SccPus30ByteArray file = { 6, "thhhat" };
	asn1SccPus30ByteArray dir = { 17, "abcdefghijklmnopq" };
	asn1SccPus30ByteArray car = { 8, "zyxwvuts" };
...
int test_client()
{
	...
	if (IS_RPI)
	{
		server = gethostbyname("172.16.201.38"); //zebra PC
	}
	else
	{
		server = gethostbyname("172.16.201.192"); //rpi2
	}
	...
	if (pusError == PUS_NO_ERROR && recv.data.u.tcData.data.u.st_5_X.arr[0].eventId == 7 && recv.data.u.tcData.data.u.st_5_X.arr[1].eventId == 10 && recv.data.u.tcData.data.u.st_5_X.arr[2].eventId == 1)
	{
