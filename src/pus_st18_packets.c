...
pusError_t pus_tc_18_1_setObcpChecksum(pusPacket_t* outTc, const asn1SccPusSt18ObcpChecksum checksum)
{
	if (NULL == outTc)
	{
		return PUS_SET_ERROR(PUS_ERROR_NULLPTR);
	...
	outTc->data.u.tcData.data.u.st_18_1.checksum = checksum;
	return PUS_NO_ERROR;
}

pusError_t pus_tc_18_1_getObcpChecksum(asn1SccPusSt18ObcpChecksum* checksum, const pusPacket_t* outTc)
{
	if (NULL == outTc || NULL == checksum)
	{
		return PUS_SET_ERROR(PUS_ERROR_NULLPTR);
	}
	if (PUS_NO_ERROR != PUS_EXPECT_ST18TC(outTc, pus_TC_18_1_loadObcpDirect))
	...
	*checksum = outTc->data.u.tcData.data.u.st_18_1.checksum;
	return PUS_NO_ERROR;
}
